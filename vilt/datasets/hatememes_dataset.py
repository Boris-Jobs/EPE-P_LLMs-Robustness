...
class HateMemesDataset(BaseDataset):
    def __init__(self, *args, split="", missing_info={}, **kwargs):
        ...
        if os.path.exists(missing_table_path):
            ...
            if missing_ratio > 0:
                missing_index = random.sample(
                    range(total_num), int(total_num * missing_ratio)
                )
                ...
                if missing_type == "text":
                    ...
                    missing_table[missing_index] = 1
                    missing_index_image = random.sample(
                        missing_index, int(len(missing_index) * both_ratio)
                    )
    ...
    def __getitem__(self, index):
        ...
        if (
            self.split == "train"
            and self.simulate_missing
            and self.missing_table[image_index] == 0
        ):
