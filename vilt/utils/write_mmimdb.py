...
def make_arrow(root, dataset_root, single_plot=False, missing_type=None):
    ...
    for split, samples in split_sets.items():
        ...
        dataframe = pd.DataFrame(
            data_list,
            columns=[
                "image",  # binary，每个条目都是JPEG二进制表示，二进制不能直接处理，得恢复到原图再进行预处理、提取特征、嵌入特征
                "plots",  # list<item: string>，情节描述
                "label",  # list<item: int32>，one-hot标签向量
                "genres",  # list<item: string>，体裁列表
                "image_id",  # string，image ID
                "split",  # string，划分信息，train、dev或test
            ],
        )
